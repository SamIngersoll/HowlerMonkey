...
class network:
    def __init__( learning_rate, max_steps, hidden1, hidden2, batch_size, input_data_dir, log_dir, data_length, data_width ):
    # Parameters
    ...
    def multilayer_perceptron( self, x, weights, biases ):
        ...
        # Hidden layer with RELU activation
        layer_2 = tf.add(tf.matmul(self.layer_1, self.weights['h2']), self.biases['b2'])
        layer_2 = tf.nn.relu(layer_2)
        # Output layer with linear activation
        out_layer = tf.matmul(self.layer_2, self.weights['out']) + self.biases['out']
        return out_layer
